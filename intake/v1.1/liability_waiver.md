# Synergize Fitness — Liability Waiver (v1.1)

> Human-readable legal record. The authoritative, hash-source copy is
> [`documents.js`](documents.js) (`canonicalText('liability_waiver')`). Keep this
> file in sync with that module; never edit one without the other.
>
> **v1.1 note:** the waiver text is **unchanged from v1.0**. It is re-versioned to
> 1.1 only because `DOC_VERSION` is a single constant applied to all three docs;
> new waiver rows store `doc_version = '1.1'` with an identical body and a freshly
> computed (identical-input) hash.

*Please confirm you have read and agree to each item below, then sign.* Each
clause is affirmed individually (the digital equivalent of initialling each
paragraph); all seven must be affirmed `true` to submit.
...
Signature: typed full name + auto-dated. No witness field (a logged digital flow
is stronger than a friend's witness signature).

Clause keys (stored affirmations): `voluntary_participation`,
`accuracy_of_disclosure`, `non_disclosure`, `assumption_of_risk`,
`supervision_questions`, `following_safety_guidance`, `termination`.
